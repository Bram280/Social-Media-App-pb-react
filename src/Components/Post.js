    ...
    useEffect(() => {
        setPostIsLiked(() => {
            ...
            for (let i = 0; i < likes; i++) {
                if (post.expand.likes_via_liked_post[i].user == pb.authStore.model.id) {
        ...
        <div className="w-full flex justify-center m-1">
            <div className={"card shadow-xl min-w-fit " + ((parent_post == "") ? "w-11/12 xl:w-3/4 bg-base-100" : "w-11/12 bg-slate-50")}>
                ...
                <div className="flex justify-around m-2">
                    <button className="w-full flex justify-center rounded-lg p-2 hover:bg-slate-100" onClick={() => {fetchComments(1,5)}}>
